...
//variables
let input;
const link = './shelter_counts_state_2016.csv';
...
//load data
d3.queue()
  .defer(d3.csv, link, parse)
  .defer(d3.json, './states.json')
  .await(function(err,counts,states){

  //map state abbrev to state names
  ...
  const selectW = d3.select('select').node().getBoundingClientRect().width;

  inputG
    .style('padding-left',((w/2)-(selectW/2))+'px')
    .style('padding-right',((w/2)-(selectW/2))+'px');

  inputStates.selectAll('option')
    .data(stateMap.keys())
    .enter()
    .append('option')
    .attr('class','options')
  ...
  function draw(input){
    ...
    const countsFilter = counts.filter(function(d){ return d.state == input});
    ...
    /*------------------------ratio------------------------*/
      ...
      ratioText
        ...
        .attr('x',(w/2)+'px')
        .attr('dy','2.4em')
        .text(`These counts are provided by ${orgs.toLocaleString('en')} animal shelters in ${stateMap.get(input)}.`);
